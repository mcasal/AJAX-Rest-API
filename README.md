# Rest API con AJAX: Chuck Norris jokes.
Creación de una página web donde se hace una llamada AJAX a una API con chistes de Chuck Norris.

## Objetivos
* Hacer una llamada a una Rest API a través de jQuery Ajax.
* Crear una interfície gráfica acorde con el contenido.
* Realizar una petición a la API utilizando la función fetch.

## Lenguajes y herramientas
<img align="left" alt="HTML5" width="35px" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/html/html.png"/>
<img align="left" alt="CSS3" width="35px" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/css/css.png"/>
<img align="left" alt="jQuery" width="35px" src="https://www.vectorlogo.zone/logos/jquery/jquery-icon.svg"/>
<img align="left" alt="Git" width="35px" src="https://www.vectorlogo.zone/logos/git-scm/git-scm-icon.svg"/>
<img align="left" alt="Visual Studio Code" width="35px" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/visual-studio-code/visual-studio-code.png"/><br/>


## Screenshots
<img width="100%" src="https://user-images.githubusercontent.com/60666104/115692588-6a906280-a35f-11eb-8e72-42468bd328a9.png"/>

